...
int main(int argc, const char* argv[]) {
  ...
  app.add_option(
         ...
         "Face model confidence threshold")
      ->default_val(0.1);
  app.add_option("-l, --licensePlateModelPath", licensePlateModelPath, "License Plate model path");
  app.add_option(
  ...
  app.add_option(
      ...
      ->default_val(1.15);
  app.add_option(
         "--nms-threshold", nmsThreshold, "NMS threshold for filtering overlapping detections")
      ->default_val(0.3);
  ...
  app.add_option(
         ...
         mutationType,
         "Mutation type:\n"
         "  - 'VERTICAL_FLIP' or 'VRS_EXPORT_REIMPORT' or 'EGOBLUR'\n"
  ...
  static const std::set<std::string> mutationsType = {
      "VERTICAL_FLIP", "VRS_EXPORT_REIMPORT", "EGOBLUR"};
  ...
  try {
    ...
    } else if (mutationType == "EGOBLUR") {
      if (setenv("ONEDNN_PRIMITIVE_CACHE_CAPACITY", "1", 1) == 0) {
        // See github issue https://github.com/pytorch/pytorch/issues/29893 for details
        std::cout << "Successfully Set ONEDNN_PRIMITIVE_CACHE_CAPACITY to 1" << std::endl;
      }
      if (setenv("TORCH_CUDNN_V8_API_DISABLED", "1", 1) == 0) {
        std::cout << "Successfully Set TORCH_CUDNN_V8_API_DISABLED to 1" << std::endl;
      }
    ...
    if (mutationType == "EGOBLUR") {
      auto* const egoBlurMutator = dynamic_cast<EgoBlur::EgoBlurImageMutator*>(imageMutator.get());
      std::cout << egoBlurMutator->logStatistics() << std::endl;
    }
    return statusCode;
  } catch (const std::exception& ex) {
    std::cerr << "Error while applying mutation : " << mutationType << " to : " << vrsPathIn
              << "\nError :\n"
              << ex.what() << std::endl;
    return EXIT_FAILURE;
  }
