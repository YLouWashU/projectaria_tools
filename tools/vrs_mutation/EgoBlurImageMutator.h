...
struct EgoBlurImageMutator : public vrs::utils::UserDefinedImageMutator {
  ...
  explicit EgoBlurImageMutator(
      ...
      : faceModelConfidenceThreshold_(faceModelConfidenceThreshold),
        licensePlateModelConfidenceThreshold_(licensePlateModelConfidenceThreshold),
        scaleFactorDetections_(scaleFactorDetections),
        nmsThreshold_(nmsThreshold),
        useGPU_(useGPU),
        clockwise90Rotation_(clockwise90Rotation) {
    device_ = getDevice();
    std::cout << "attempting to load ego blur face model: " << faceModelPath << std::endl;

    if (!faceModelPath.empty()) {
      faceModel_ = loadModel(faceModelPath);
    }

    std::cout << "attempting to load ego blur license plate model: " << licensePlateModelPath
              << std::endl;
  ...
  std::shared_ptr<torch::jit::script::Module> loadModel(const std::string& path) {
  ...
  torch::Tensor filterDetections(
    ...
    torch::Tensor scoreThresholdMask =
        torch::gt(detections->elements().at(2).toTensor(), torch::tensor(scoreThreshold)).detach();
    // we have boxes at index 0
    torch::Tensor filteredBoundingBoxes =
        detections->elements().at(0).toTensor().index({scoreThresholdMask}).detach();
    torch::Tensor filteredBoundingBoxesScores =
        detections->elements().at(2).toTensor().index({scoreThresholdMask}).detach();

    // filter out overlapping detections by performing NMS
    torch::Tensor filteredBoundingBoxesPostNMS =
        performNMS(filteredBoundingBoxes, filteredBoundingBoxesScores, nmsThreshold_).detach();
  ...
  torch::Tensor performNMS(
    ...
    torch::Tensor filteredBoundingBoxes =
        torch::index_select(
            ...
            0,
            torch::from_blob(
                selectedIndices.data(), {static_cast<long>(selectedIndices.size())}, torch::kInt))
            .detach();
  ...
  scaleBox(const std::vector<float>& box, int maxWidth, int maxHeight, float scale) {
  ...
  cv::Mat
  blurImage(const cv::Mat& image, const std::vector<torch::Tensor>& detections, float scale) {
    ...
    for (const auto& detection : detections) {
      for (auto& box : detection.unbind()) {
        std::vector<float> boxVector(box.data_ptr<float>(), box.data_ptr<float>() + box.numel());
        if (scale != 1.0f) {
        ...
        blurredImage.copyTo(response(cv::Rect({x1, y1, w, h})), mask(cv::Rect({x1, y1, w, h})));
  ...
  cv::Mat detectAndBlur(vrs::utils::PixelFrame* frame, const std::string& frameId) {
    ...
    cv::Mat img =
        cv::Mat(height, width, CV_8UC(channels), static_cast<void*>(frame->getBuffer().data()))
            .clone();
    ...
    torch::Tensor imgTensor =
        torch::from_blob((void*)frame->rdata(), {height, width, channels}, torch::kUInt8);
    ...
    if (clockwise90Rotation_) {
      imgTensorFloatOnDevicePostRotation = torch::rot90(imgTensorFloatOnDevice, -1);
    } else {
    ...
    std::vector<torch::jit::IValue> inputs = {imgTensorFloatOnDevicePostRotationCHW};
    ...
    if (faceModel_) {
      c10::intrusive_ptr<c10::ivalue::Tuple> faceDetections =
          faceModel_->forward(inputs).toTuple(); // returns boxes, labels, scores, dims
      faceBoundingBoxes = filterDetections(faceDetections, faceModelConfidenceThreshold_);
    ...
    if (licensePlateModel_) {
      c10::intrusive_ptr<c10::ivalue::Tuple> licensePlateDetections =
          licensePlateModel_->forward(inputs).toTuple(); // returns boxes, labels, scores, dims
      licensePlateBoundingBoxes =
          filterDetections(licensePlateDetections, licensePlateModelConfidenceThreshold_);
      int totaLlicensePlateDetectionsForCurrentFrame = licensePlateBoundingBoxes.sizes()[0];
      stats_[frameId]["licensePlate"] += totaLlicensePlateDetectionsForCurrentFrame;
  ...
  bool operator()(double timestamp, const vrs::StreamId& streamId, vrs::utils::PixelFrame* frame)
    ...
    if (streamId.getNumericName().find("214") != std::string::npos ||
      ...
      std::string frameId = streamId.getNumericName() + "_" + std::to_string(timestamp);
    ...
    if (!blurredImage.empty()) {
      // RGB
      if (streamId.getNumericName().find("214") != std::string::npos) {
        std::memcpy(frame->wdata(), blurredImage.data, frame->getWidth() * frame->getStride());
      }
      // Gray
      else if (streamId.getNumericName().find("1201") != std::string::npos) {
        std::memcpy(frame->wdata(), blurredImage.data, frame->getWidth() * frame->getHeight());
      }
    }
    blurredImage.release();
  ...
  std::string logStatistics() const {
    ...
    for (const auto& outer : stats_) {
      ...
      const std::unordered_map<std::string, int>& categoryBoxCountMapping = outer.second;
