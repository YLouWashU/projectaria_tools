...
class Downloader(RunnerWithProgress):
    ...
    semaphore_: Final[Semaphore] = Semaphore(
        value=config.getint(DOWNLOAD, CONCURRENT_DOWNLOADS)
    )
    ...
    @retry(
        ...
        interval=config.getfloat(DOWNLOAD, INTERVAL),
        backoff=config.getfloat(DOWNLOAD, INTERVAL),
    )
    async def run(self) -> None:
        ...
        async with self.semaphore_:
            async with self._http_helper.get(self._url) as response:
                ...
                chunk_size: int = config.getint(DOWNLOAD, CHUNK_SIZE)
        ...
        if self._unzip:
            ...
            if config.getboolean(DOWNLOAD, DELETE_ZIP):
