...
class RequestMonitor(BaseStateMachine):
    ...
    async def track_feature_request(self, *args, **kwargs) -> None:
...
class RequestMonitorModel:
    ...
    def get_status(self, recording: Path) -> str:
        ...
        if self.is_CREATE():
            ...
            return ModelState(status=DOWNLOADING, progress=progress)
    ...
    async def on_enter_WAIT(self, event: EventData) -> None:
        logger.debug(event)
        status_check_interval: int = config.getint(DEFAULT, STATUS_CHECK_INTERVAL)
    ...
    async def _download_multi_slam_summary(self) -> None:
        ...
        if self.feature == MpsFeature.MULTI_SLAM:
            ...
            # Create results.json file to map
            results_dict: Dict[str, Any] = {
                str(r.path): str(r.output_path) for r in self._recordings
            }
            results_json: Path = self._recordings[0].output_path.parent / "results.json"
            results_json.parent.mkdir(parents=True, exist_ok=True)
            with results_json.open("w") as f:
                json.dump(results_dict, f, indent=2)
            logger.debug("Wrote results.json")
