...
class Uploader(RunnerWithProgress):
    ...
    @retry(
        ...
        retries=config.getint(UPLOAD, RETRIES),
        interval=config.getfloat(UPLOAD, INTERVAL),
        backoff=config.getfloat(UPLOAD, BACKOFF),
    )
    async def run(self) -> int:
        ...
        async with Uploader.semaphore_:
            ...
            min_chunk_size: int = config.getint(UPLOAD, MIN_CHUNK_SIZE)
            max_chunk_size: int = config.getint(UPLOAD, MAX_CHUNK_SIZE)
            smoothing_window_size: int = config.getint(UPLOAD, SMOOTHING_WINDOW_SIZE)
            target_chunk_upload_secs: int = config.getint(
                UPLOAD, TARGET_CHUNK_UPLOAD_SECS
            )
