...
class SingleRecordingModel:
    ...
    def get_status(self, feature: MpsFeature) -> ModelState:
        ...
        if feature in self._ineligible_features:
        ...
        elif self.is_EXISTING_REQUESTS_CHECK() or self.is_EXISTING_RECORDING_CHECK():
            return ModelState(status=CHECKING)
            ...
            return ModelState(status=UPLOADING, progress=progress)
        elif self.is_SUBMIT():
    ...
    async def on_enter_VALIDATION(self, event: EventData) -> None:
        self._logger.debug(event)
        if not self._force and self._recording.health_check_path.exists():
        ...
        if not self._features:
            self._error_code = ERROR_HEALTH_CHECK
        await self.next()
    ...
    async def on_enter_UPLOAD(self, event: EventData) -> None:
        ...
        if config.getboolean(ENCRYPTION, DELETE_ENCRYPTED_FILES):
            self._logger.info(
